...
final_update = 3
header = "uid smoi sym_int rep update host_count sym_count sym_val host_val burst_size cfu\n"
...
for t in treatment_postfixes:
 ...
 #           host_fname = folder +"HostVals_VT"+str(t)+"_Seed" + str(r)+".data"
            ...
            with open(host_fname) as host_file, open(sym_fname) as sym_file:
                for host_line, sym_line, lysis_line in zip(host_file, sym_file, lysis_file):
                    if (host_line[0] != "u"):
